...
__name(getSkillManifest, "getSkillManifest");
function loadSkills(skillNames) {
  ...
  for (const name of skillNames) {
    ...
    if (skill) {
      ...
      logger.info(`Loaded skill: ${name}`, { size: skill.content.length });
    } else {
...
function formatManifestForSelector(manifest) {
  return manifest.map((s) => `- **${s.name}**: ${s.description}`).join("\n");
}
__name(formatManifestForSelector, "formatManifestForSelector");

export {
  getSkillManifest,
  loadSkills,
  formatManifestForSelector
};
//# sourceMappingURL=chunk-BGAJKKYA.mjs.map
